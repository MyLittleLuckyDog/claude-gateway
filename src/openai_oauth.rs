...
impl OpenAiOAuthState {
    ...
    async fn refresh_tokens(&self) -> Result<(), GatewayError> {
        ...
        let json: Value = resp
            .json()
            .await
            .map_err(|e| GatewayError::Internal(format!("OAuth refresh response decode failed: {e}")))?;
...
pub fn normalize_codex_body(mut body: Value) -> Value {
    if let Some(obj) = body.as_object_mut() {
        obj.entry("instructions").or_insert_with(|| Value::String(String::new()));
    ...
    fn apply_event(&mut self, value: &Value) {
        match value["type"].as_str().unwrap_or_default() {
            "response.created" | "response.in_progress" | "response.completed" => {
                ...
                self.id = response["id"].as_str().map(str::to_string).or(self.id.take());
                self.model = response["model"].as_str().map(str::to_string).or(self.model.take());
            ...
            "response.output_item.done" => {
                ...
                match item["type"].as_str().unwrap_or_default() {
                    "message" => {
                        self.message_id = item["id"].as_str().map(str::to_string).or(self.message_id.take());
                    ...
                    "function_call" => {
                        self.tool_calls.push(response_function_item_to_chat_message(item));
    ...
    fn to_responses_json(&self) -> Value {
        ...
        if let Some(obj) = response.as_object_mut() {
            ...
            if output_missing_or_empty {
                obj.insert("output".to_string(), Value::Array(self.to_responses_output()));
...
async fn collect_response_sse(resp: reqwest::Response) -> Result<ResponseAccumulator, GatewayError> {
    let text = resp
        .text()
        .await
...
pub fn chat_to_responses_body(body: Value, _stream: bool) -> Result<Value, GatewayError> {
    ...
    if let Some(out_obj) = out.as_object_mut() {
        ...
        if let Some(tool_choice) = obj.get("tool_choice") {
            out_obj.insert("tool_choice".to_string(), chat_tool_choice_to_response(tool_choice));
        }
...
fn extract_text_from_response_message(item: &Value) -> Option<String> {
    item["content"].as_array().map(|content| {
        content
            .iter()
...
fn resolve_auth_file_path() -> Option<PathBuf> {
    ...
    let candidates: Vec<PathBuf> = [
        ...
        std::env::var("CODEX_HOME")
            .ok()
            .map(|h| PathBuf::from(h).join("auth.json")),
        home.as_ref().map(|h| h.join(".chatgpt-local").join("auth.json")),
...
mod tests {
    ...
    fn normalize_preserves_existing_instructions() {
        let body = serde_json::json!({"model": "gpt-4o", "input": "hi", "instructions": "be concise"});
        let out = normalize_codex_body(body);
    ...
    fn response_accumulator_maps_function_call_to_chat_tool_calls() {
        ...
        assert_eq!(out["choices"][0]["message"]["tool_calls"][0]["id"], "call_123");
        assert_eq!(out["choices"][0]["message"]["tool_calls"][0]["function"]["name"], "extract_heading");
        ...
        assert_eq!(responses["output"][0]["arguments"], "{\"heading\":\"Example Domain\"}");
    }
    ...
    fn response_accumulator_synthesizes_responses_message_schema() {
        ...
        assert_eq!(out["output"][0]["content"][0]["annotations"].as_array().unwrap().len(), 0);
        assert_eq!(out["output"][0]["content"][0]["logprobs"].as_array().unwrap().len(), 0);
    }
    ...
    fn extract_account_id_returns_value_when_present() {
        let payload = r#"{"sub":"user123","https://api.openai.com/auth.chatgpt_account_id":"acct_abc123"}"#;
        let encoded = base64url_encode(payload.as_bytes());
        let fake_jwt = format!("header.{encoded}.signature");
        let result = extract_account_id_from_jwt(&fake_jwt);
    ...
    async fn from_auth_file_loads_valid_file() {
        ...
        tokio::fs::write(&path, serde_json::to_string(&content).unwrap()).await.unwrap();
    ...
    async fn from_auth_file_returns_none_for_missing_file() {
        ...
        unsafe {
            ...
            std::env::set_var("CHATGPT_LOCAL_HOME", "/tmp/claude_gateway_test_nonexistent_xyz");
        }
        let state = OpenAiOAuthState::from_auth_file().await;
        unsafe {
    ...
    async fn ensure_fresh_token_returns_tokens_when_not_expired() {
        ...
        tokio::fs::write(&path, serde_json::to_string_pretty(&content).unwrap()).await.unwrap();
