...
fn build_headers(
    ...
    headers.insert("x-stainless-runtime-version", HeaderValue::from_static("v23.11.0"));
...
pub async fn messages_sync(
    ...
    if let Some(msg) = state.pre_flight_check().await {
        ...
        return Err(ProxyError::RateLimited { message: Some(msg), resets_at });
    }

    let token = auth::get_oauth_token()
        .map_err(|e| ProxyError::Auth { message: e })?;
    if !auth::is_token_valid(&token) {
        return Err(ProxyError::Auth {
            message: "OAuth token expired. Run `claude /login` to refresh.".to_string(),
        });
    }
    ...
    Err(ProxyError::Transport(format!("Request failed after retries (last status: {last_status})")))
}
...
pub async fn messages_stream(
    ...
    // Pre-flight: check cached rate limit
    if let Some(msg) = state.pre_flight_check().await {
        return Err(ProxyError::RateLimited { message: Some(msg), resets_at: None });
    }
